...
const footerNode = document.getElementsByTagName("footer")[0];
let menuOpen = false;
...
footerNode.innerHTML = `<p>© 2025 PYQtest.</p>
    <p>(●'◡'●)</p>
    <div style="display: flex; justify-content: center; align-items: center; gap: 8px;">
      <span>
      <svg xmlns="http://www.w3.org/2000/svg" fill="#000000" width="20px" height="20px" viewBox="0 -0.5 25 25">
      ...
      <a href="https://github.com/pushpiyush" target="_blank">pushpiyush</a>
  </div>`;

// Create and append off canvas menu modal element right after header node
const menuModalNode = document.createElement("article");
menuModalNode.id = "menu-modal";
menuModalNode.classList.add("hide");
menuModalNode.setAttribute("onclick", "menuModalClick(event)");
menuModalNode.setAttribute("onkeydown", "menuModalEsc(event)"); // having problem here *e2
headerNode.append(menuModalNode);

// Inject menu modal HTML content
...
// DOM Element References after injection
...
function toggleMenu() {
  ...
  if (menuOpen) {
    // Close menu
    menuNode.style.animation = "slideleft 0.4s ease-in-out";
    setTimeout((() => c.add("hide")), 400);
    ...
    if (c.contains("hide")) {
      c.remove("hide");
      menuNode.style.animation = "slideright 0.4s ease-in-out";
      menuOpen = !menuOpen;
    }
  }
}
...
// having problem here *e2
function menuModalEsc(event){
  console.log("esc pressed");
  toggleMenu();
}

// TODO: replace setTimeout with animationend event listener
// Handles the rotation and icon change of the menu button
function menuAnimation() {
  if (menuOpen) {
    // Animate to 'menu' icon
    headerMenuNode.style.animation = "rotate-clock 0.2s ease-in";
    setTimeout((() => {
      headerMenuNode.innerHTML = "menu";
      headerMenuNode.style.animation = "rotate-clock2 0.2s ease-out";
    }), 200);
  } else {
    // Animate to 'close' icon
    headerMenuNode.style.animation = "rotate-anticlock 0.2s ease-in";
    setTimeout((() => {
      headerMenuNode.innerHTML = "close";
      headerMenuNode.style.animation = "rotate-anticlock2 0.2s ease-out";
    }), 200);
  }
}
...
// Toggles the theme selection modal
